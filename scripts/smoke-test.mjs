...
const compile=async(src,name)=>{let s=await fs.readFile(new URL('../'+src,import.meta.url),'utf8');s=s.replace(/['"](?:@\/lib\/|\.\/)(server|auth|session-crypto|image-validation|game-data|search-engine)['"]/g,"'./$1.mjs'").replace("'next/headers'","'./cookies.mjs'");await fs.writeFile(path.join(tmp,name+'.mjs'),ts.transpileModule(s,{compilerOptions:{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022}}).outputText);};
...
let role='viewer',status='approved',network=0,writes=0,queries=[],memberExists=true;let corpusTotal=361;
...
try{
 ...
 for(const n of ['server','auth','session-crypto','image-validation','game-data','search-engine'])await compile('lib/'+n+'.ts',n);
 ...
 await search.POST(req({q:'faq_001',category:'Account',page:2,pageSize:48,game:'any-game'}));const q=queries.at(-1);assert.equal(q.searchParams.get('offset'),'0');assert.equal(q.searchParams.get('game'),'eq.any-game');assert.equal(q.searchParams.has('or'),false);const suggested=await (await search.POST(req({q:'เติมเงิน',suggest:true}))).json();assert.equal(suggested.entries.length,6);assert.equal(suggested.total,361);assert.equal(suggested.entries[0].match_kind,'exact');assert.equal((await search.POST(req({q:'x'.repeat(501)}))).status,400);
 corpusTotal=1361;queries=[];const scanned=await (await search.POST(req({q:'เติมเงิน',page:15,pageSize:96}))).json();assert.equal(scanned.total,1361);assert.equal(scanned.entries.length,17);assert.deepEqual(queries.map(u=>u.searchParams.get('offset')),['0','500','1000']);corpusTotal=361;
 console.log('PASS pending/blocked access, viewer cannot write, 361-record browsing, 1,361-record ranked search across batches, Thai suggestions');
